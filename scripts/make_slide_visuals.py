...
VTU_WORKSTATION = (
    PROJECT_ROOT / "scripts/su2_fine_runs/real_workstation/vol_solution.vtu"
)
...
def _iso_panel(vtu_path: Path, out_png: Path, title: str, size=(1100, 800)):
    ...
    field = (
        ...
        if "Pressure_Coefficient" in surf.point_data
...
def make_mesh_compare():
    ...
    for vtu, label, n_cells, ld in (
        ...
        (
            VTU_WORKSTATION,
            "workstation preset",
            "11.3 k surf / ~100 k vol  -  L/D = 20.06",
            None,
        ),
        (
            VTU_INDUSTRY,
            "industry preset",
            "54.5 k surf / ~400 k vol  -  L/D = 17.16",
            None,
        ),
    ):
    ...
    caption = (
        "Same D150, same flight point (M 0.78 / AoA 2.5deg / FL350).  "
...
def make_benchmark_loss():
    ...
    fig.suptitle(
        "Hybrid matches the best solo loss while keeping image-grounded multimodal verdicts",
        fontsize=13,
        fontweight="bold",
        y=1.02,
    )
...
def make_benchmark_categories():
    ...
    ax.set_title(
        "Per-category scores  -  hybrid takes the best of both models",
        fontsize=13,
        pad=10,
    )
...
def make_arch_diagram():
    ...
    for name, sub, cx, cy in mcps:
        ...
        ax.text(
            ...
            fontsize=9,
            color="#4b5563",
            style="italic" if is_planned else "normal",
        )
        # connector line to the bus
        bus_cx, bus_cy = 6.5, 3.5
        ax.plot(
            [cx, bus_cx],
            [cy + (0.55 if cy < 3 else -0.55), bus_cy],
            color="#94a3b8",
            linewidth=1.2,
            zorder=0,
        )
    ...
    ax.text(
        0.2,
        0.3,
        "*  planned / candidate.  agent box is the orchestrator.",
        fontsize=9,
        color="#6b7280",
        style="italic",
    )
...
def make_hybrid_flow():
    ...
    def box(cx, cy, w, h, title, sub, face, edge):
        ...
        ax.text(
            cx, cy - 0.28, sub, ha="center", va="center", fontsize=10, color="#4b5563"
        )
    ...
    box(1.5, 5.4, 2.4, 0.9, "User prompt", "natural language", "#f3f4f6", "#6b7280")
    box(
        5.0,
        5.4,
        3.2,
        1.1,
        "Planner",
        "Qwen 2.5 7B  -  native tool-calling",
        "#dbeafe",
        "#1d4ed8",
    )
    box(
    ...
    box(
        ...
        3.2,
        1.1,
        "render_aircraft_views",
        "3-panel Cp composite (iso / top / side)",
        "#fef3c7",
        "#a16207",
    )
    box(
        ...
        1.1,
        "Seeker",
        "Gemma 4 E4B  -  multimodal verdict",
        "#dcfce7",
        "#15803d",
    )
    ...
    box(
        ...
        3.2,
        1.1,
        "Planner refines or finalises",
        "report_done with CL/CD/L/D + verdict",
        "#dbeafe",
        "#1d4ed8",
    )
    ...
    fig.suptitle(
        "Hybrid agent  -  Planner / Seeker / Answer-Agent pattern",
        fontsize=14,
        fontweight="bold",
        y=1.01,
    )
...
def make_amr_ladder():
    ...
    fig.suptitle(
        "Adaptive mesh refinement skill  -  agent escalates fidelity until lift plateaus",
        fontsize=14,
        fontweight="bold",
        y=1.02,
    )

...
Goal: drive the full pipeline (geometry -> aero -> engine -> mission)
from natural language in front of the Boeing team, with **Gemma 4
visually verifying the SU2 output** mid-flow. ~15 minutes if every
command lands cleanly.

> **Pre-meeting**: 30 minutes before, run **Block 0** end-to-end so
> Ollama has the Gemma model hot and SU2 has cached meshes. Cold-start
...
> `demo_recording_<date>.mov`. Play that if WiFi or Ollama acts up.
...
# 1. Confirm Ollama + Gemma model
brew services list | grep ollama
ollama list                                   # expect: gemma4:e4b (and optionally gemma3:27b on server tier)

# 2. Warm the model in memory
...
local, open-weight, enterprise-friendly licensing."*

---

## Block 2 -- Hybrid agent, one shot (~3 min)

This is the headline. **Gemma 4 plans, Gemma 4 sees, both run on a
16 GB MacBook with no cloud calls.**
...
## Block 4 -- Open-ended mesh convergence (~3 min, new)

This is the headline for the 2026-06-21 update. The user asks for a
*converged* SU2 result and the agent keeps refining the mesh until
CL/CD plateau, instead of stopping at a fixed preset.

```bash
# Deterministic harness (no LLM; reproducible numbers for the slide):
python scripts/run_converged_su2.py \
  --cpacs D150_v30.xml \
  --step pipeline/d150_final/aircraft_fused.step \
  --start-density 30 --growth 2.0 --max-rungs 5 \
  --max-wall-seconds 5400 \
  --mach 0.78 --aoa 2.5 --altitude 35000

# Agent-driven version (same algorithm, planner emits the rung table):
python agent-mcp/hybrid_agent.py --cpacs D150_v30.xml \
  --prompt "Deliver a converged SU2 result for D150 at Mach 0.78 AoA 2.5 FL350. \
            Use the open-ended mesh refinement skill (SKILL_OPEN_ENDED_MESH.md). \
            Stop when |dCL|/CL < 1% and |dCD|/CD < 1% over consecutive meshes \
            AND the inner Cauchy criterion has fired. Cap at 5 rungs, 90 minutes."
```

Show the printed rung table on screen: density, n_elem, CL, CD, L/D,
Cauchy flag, runtime. The converged number is the headline.

---

## Block 5 -- Solo backend for contrast (~2 min)

```bash
# Plain single-model Gemma, no seeker, for comparison
...
is the single-model baseline. The hybrid adds the Gemma visual
verification layer for free."*

---

## Block 6 -- Benchmark transparency (~2 min)

```bash
cd agentic-bench
cat reports/hybrid_combined_FIXED.json       | jq '.aggregate'
cat reports/gemma4_e4b_multimodal_FIXED.json | jq '.aggregate'
cat reports/qwen2_5_7b_combined.json         | jq '.aggregate'  # historical comparison
cd ..
```
...
- **Single Gemma 4 multimodal: 0.333**, image-grounded baseline.
- **Solo Qwen combined loss: 0.165** (historical), multimodal = 0.67
  but *blind* (no vision -- guessed "acceptable" twice on a 3-item
  suite).

Talk track: *"We retired Qwen as the default because Gemma 4 reaches
parity on routing/numerical with native function calling, lets us stay
in a single open-weight model family, and keeps the licensing story
clean for industry partners. The historical Qwen numbers are still in
the repo for transparency."*
...
- 3 agents live: hybrid (production all-Gemma), `gemma_agent`
  ...
  Gemma 3).
- Open-ended mesh refinement: agent or deterministic harness.
- One-command installer: `bash agent-mcp/bootstrap.sh`.
...
# Pre-flight
source .venv/bin/activate
ollama list && ollama run gemma4:e4b "warmup"
...
# Headline: open-ended convergence
python scripts/run_converged_su2.py --cpacs D150_v30.xml \
  --step pipeline/d150_final/aircraft_fused.step \
  --mach 0.78 --aoa 2.5 --altitude 35000 \
  --start-density 30 --growth 2.0 --max-rungs 5
